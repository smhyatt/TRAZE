...
import { TransformationDB, TransformationInputDB, TransformationOutputDB, uid } from '../types'
...
const {
    ...
    getTransformations,
    getTransformationsByOwner,
} = require('../database/queries')

export class TransformationRepo {

    async addTransformation(input) {
        ...
        await db.many(query1).then(data => {return data})

        const query2 = pgp.helpers.concat([inputQuery, outputQuery])
        return await db.many(query2).then(data => {return data})
    } 


    async selectTransformationById(transformationId : uid) : Promise<TransformationDB> {
        return await db.any(getTransformation, transformationId).then(transformation => { return transformation })
    }

    async selectTransformationInputById(transformationId : uid) : Promise<TransformationInputDB> {
        return await db.any(getTransformationInput, transformationId).then(transformation => { return transformation })
    }

    async selectTransformationOutputById(transformationId : uid) : Promise<TransformationOutputDB> {
        return await db.any(getTransformationOutput, transformationId).then(transformation => { return transformation })
    }

    async selectTransformations() {
        return await db.any(getTransformations).then(transformations => { return transformations })
    }

    async selectTransformationsByOwner(ownerId : uid) {
        return await db.any(getTransformationsByOwner, ownerId).then(async transformations => { return transformations })
    }
